...
void yncc_gtk::Yncc_Gtk_Window::connectDevice(const std::string& host) {
    if(yncapi::deviceAssign(host)) {
        connected = true;

        for(unsigned int i = 0; i < deviceList.devices.size(); ++i) {
            if(deviceList.devices[i].DeviceIP == host)
...
void yncc_gtk::Yncc_Gtk_Window::autoConnect() {
    for(unsigned int i = 0; i < deviceList.devices.size(); ++i)
        if(deviceList.devices[i].AutoConnect)
            connectDevice(deviceList.devices[i].DeviceIP);
}
